...
export async function PUT(request: Request) {
  try {
    ...
    return NextResponse.json({ message: "Pengaturan pendidikan berhasil disimpan." });
  } catch (error) {
    console.error(error);
    return NextResponse.json({ message: "Gagal menyimpan pengaturan pendidikan." }, { status: 500 });
  }
}