...
export async function PUT(request: Request) {
  try {
    ...
    return NextResponse.json({ message: "Pengaturan sertifikat berhasil disimpan." });
  } catch (error) {
    console.error(error);
    return NextResponse.json({ message: "Gagal menyimpan pengaturan sertifikat." }, { status: 500 });
  }
}