...
const Banner = ({ locale: propLocale }: BannerProps) => {
  ...
  const profileCard = (
    ...
    >
      ...
      <Button
        onClick={() => handleDownload(cvFileUrl)}
        className={`${isNeo ? "border-[3px] border-black bg-amber-400 text-black shadow-[4px_4px_0px_0px_black] hover:brightness-110" : isRetro ? "border border-gray-300 bg-white text-black hover:bg-gray-100" : "bg-white hover:bg-gray-300 text-black"} p-4 rounded-full text-lg my-4`}
      >
        Download CV
  ...
  return (
    <section className="w-full py-4 md:pb-20 md:px-8" id="home">
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
        <div className="col-span-3 md:col-span-1 p-4 md:pr-10">{isNeo || isRetro ? profileCard : <HoverBorderGradient>{profileCard}</HoverBorderGradient>}</div>
        <div className="col-span-3 md:col-span-2 p-4">
          <div className="flex flex-col md:flex-row gap-4">
            ...
            <div
              className={`flex w-full md:w-auto flex-col gap-8 md:gap-10 items-start md:items-center justify-start h-auto self-center p-6 ${isNeo ? "border-2 border-black bg-amber-400" : isRetro ? "bg-white border border-gray-300" : "bg-black rounded-3xl"}`}
            >
