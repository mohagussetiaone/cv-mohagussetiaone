...
const Banner = ({ locale: propLocale }: BannerProps) => {
  ...
  const websiteUrl = t2.getGlobal("websiteUrl") ?? "https://mohagussetiaone.vercel.app";
  ...
  const bannerImage = t2.getGlobal("bannerImage") ?? "/assets/image/profile/mohagus.jpeg";

  const profileCard = (
    <div className={`${isNeo ? "border-[3px] border-black p-6 shadow-[6px_6px_0px_0px_black]" : isRetro ? "border border-black p-6" : "border border-gray-500 p-6 rounded-tl-[8rem] rounded-br-[8rem]"}`}>
      <div className="flex flex-col items-center justify-center mx-auto py-6">
        <Image src={bannerImage} width={220} height={180} alt="BannerImage.jpg" className="object-contain rounded-full" />
      </div>
      <div className="flex flex-col gap-4 py-4">
        <div className="flex gap-2">
          <Mail className="w-6 h-6 text-brand-500" />
          <h1 className={isRetro ? "text-black" : "text-white"}>{email}</h1>
        </div>
        <div className="flex gap-2">
          <MapPin className="w-6 h-6 text-brand-500" />
          <h1 className={`text-start ${isRetro ? "text-black" : "text-white"}`}>{address}</h1>
        </div>
        <div className="flex gap-2">
          <Briefcase className="w-6 h-6 text-brand-500" />
          <h1 className={isRetro ? "text-black" : "text-white"}>{jobTitle}</h1>
        </div>
        <div className="flex gap-2">
          <Link2 className="w-6 h-6 text-brand-500" />
          <h1 className={isRetro ? "text-black" : "text-white"}>{websiteUrl}</h1>
        </div>
      </div>
      <Button onClick={() => handleDownload(cvFileUrl)} className={`${isNeo ? "border-[3px] border-black bg-yellow-400 text-black shadow-[4px_4px_0px_0px_black]" : isRetro ? "border border-black bg-white text-black" : "bg-white hover:bg-gray-300 text-black"} p-4 rounded-full text-lg my-4`}>
        Download CV
        ...
        <div className="col-span-3 md:col-span-2 p-4">
          <div className="flex flex-col md:flex-row gap-4">
            <div className="flex flex-col gap-4 items-start justify-start md:mx-6">
              ...
              <h1 className={`text-5xl ${isRetro ? "text-black" : "text-white"}`}>{t2.get("greeting")}</h1>
              <h1 className={`text-4xl ${isRetro ? "text-black" : "text-white"}`}>{t2.get("name")}</h1>
              <TypewriterEffect words={words} />
              <span className="text-sm text-brand-500">{"<h1/>"}</span>
              <div>
                <span className="text-sm text-brand-500">{"<p>"}</span>
                <p className={`text-lg ${isRetro ? "text-black" : "text-white"}`}>{t2.get("description")}</p>
                <span className="text-sm text-brand-500">{"<p/>"}</span>
              </div>
              <div className="py-4 text-brand-500">
                <Link href={`https://wa.me/${whatsappNumber}`} className="flex gap-2 text-3xl" target="_blank">
                  {t2.get("letsTalk")}
                  <span className={`flex flex-col items-center justify-center rounded-full px-2 ${isNeo ? "border-2 border-black bg-yellow-400" : isRetro ? "border border-black bg-white" : "bg-black"}`}>
                    <Phone className="w-6 h-6" />
                  </span>
                </Link>
              </div>
            </div>
            <div className={`flex w-full md:w-auto flex-col gap-8 md:gap-10 items-start md:items-center justify-start h-auto self-center p-6 ${isNeo ? "border-2 border-black bg-yellow-200" : isRetro ? "border border-black bg-white" : "bg-black rounded-3xl"}`}>
              <div className="flex gap-3">
                <div className="h-full flex justify-center items-center">
                  <h1 className="text-4xl text-brand-500">{yearsExperience}</h1>
                </div>
                <div className={`flex flex-col ${isRetro ? "text-black" : "text-white"}`}>
                  <h2>{t2.get("years")}</h2>
