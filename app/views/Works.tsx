...
const Works: React.FC<WorksProps> = ({ locale, projects }) => {
    ...
    <div className="py-4 md:py-24 px-4 md:px-24" id="portfolio">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 xl:mx-auto">
        {projects.slice(0, visibleProducts).map((project, index) => (
          <Link key={project.productId} href={`/${locale}/project/${project.productId}`}>
            <div
            ...
            >
              <div className="relative flex h-32 xl:h-40 overflow-hidden">
                <Image className="object-cover" width={250} height={140} src={project.image || ""} alt={project.projectName} />
              </div>
              <div className="mt-2 md:mt-2 px-2 md:px-2 pb-2 md:pb-2">
                <div>
                  <div className="flex flex-col text-left text-neutral-100">
                    <h2 className="text-sm md:text-base font-bold line-clamp-1">{project.projectName}</h2>
