...
function EducationCard({ edu, index, theme, localizedText }: { edu: EducationType; index: number; theme: string; localizedText: Record<string, string> }) {
  const field = localizedText[`item_${index + 1}.field`] ?? edu.field;
  const description = localizedText[`item_${index + 1}.description`] ?? edu.description ?? "";
  return (
    <div
      className={cn(
    ...
    >
      ...
      <div
        className={cn(
          ...
          theme !== "neobrutalism" && theme !== "retro" && "border border-white/10 bg-linear-to-br from-sky-500/20 to-sky-500/5",
        )}
      >
        <GraduationCap className={cn("h-6 w-6", theme === "neobrutalism" && "text-black", theme === "retro" && "text-[#6699ff]", theme !== "neobrutalism" && theme !== "retro" && "text-sky-400")} />
      </div>
      ...
      <div className="min-w-0 flex-1">
        <div className="flex items-start justify-between gap-2">
          <div>
            ...
            <p className={cn("text-sm font-medium", theme === "neobrutalism" && "text-amber-600", theme === "retro" && "text-[#6699ff]", theme !== "neobrutalism" && theme !== "retro" && "text-sky-400")}>
              {edu.degree} &mdash; {field}
            </p>
          </div>
        </div>
...
const Education = () => {
  ...
  const title = useMemo(() => content.localized["title"] || "Education", [content.localized]);
  const description = useMemo(() => content.localized["description"] || "My educational background", [content.localized]);
  ...
  return (
    <div className="py-10 md:py-20 px-4 md:px-8" id="education">
      ...
      <div className="relative flex flex-col items-center text-center mb-12">
        <h2 className={cn("text-center text-4xl underline", theme === "neobrutalism" && "text-amber-400", theme === "retro" && "text-[#6699ff]", theme !== "neobrutalism" && theme !== "retro" && "text-brand-500")}>{title}</h2>
        <p className={cn("mt-2 max-w-2xl", theme === "neobrutalism" ? "text-black/60" : theme === "retro" ? "text-black/60" : "text-white/50")}>{description}</p>
        <div
          className={cn(
      ...
      {education.length > 0 && (
        <div className="max-w-3xl mx-auto space-y-4">
          {education.map((edu, i) => (
            <EducationCard key={edu.id} edu={edu} index={i} theme={theme} localizedText={content.localized} />
          ))}
        </div>
      )}
    </div>
